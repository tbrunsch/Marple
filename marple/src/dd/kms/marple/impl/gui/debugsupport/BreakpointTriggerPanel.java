...
class BreakpointTriggerPanel extends JPanel implements Disposable
{
	...
	BreakpointTriggerPanel(Consumer<Throwable> exceptionConsumer, InspectionContext context) {
		super(new GridBagLayout());

		debugSettings = context.getSettings().getDebugSettings();

		customMethodTF = new ExpressionInputTextField(context);
		customMethodTF.setExceptionConsumer(exceptionConsumer);
		customMethodPanel = new EvaluationTextFieldPanel(customMethodTF, context);
